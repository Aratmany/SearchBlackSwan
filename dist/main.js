...
var CanvasForceGraph = Kapsule__default['default']({
  ...
  update: function update(state) {
    ...
    state.forceLayout.stop().alpha(1) 
...
var forceGraph = Kapsule__default['default']({
  ...
  stateInit: function stateInit() {
    return {
      ...
      colorTracker: new ColorTracker__default['default']() 
  ...
  init: function init(domNode, state) {
    ...
    d3Selection.select(state.canvas).call(d3Drag.drag().subject(function () {
      ...
      state.forceGraph.d3AlphaTarget(0) 
      .resetCountdown(); 
    ...
    state.zoom(state.zoom.__baseElem = d3Selection.select(state.canvas)); 
    ...
    ['pointermove', 'pointerdown'].forEach(function (evType) {
      return container.addEventListener(evType, function (ev) {
        ...
        !state.isPointerDragging && ev.type === 'pointermove' && state.onBackgroundClick 
