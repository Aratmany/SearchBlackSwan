...
export default Kapsule({
  ...
  props: {
    ...
    dagMode: { onChange(dagMode, state) { 
      !dagMode && (state.graphData.nodes || []).forEach(n => n.fx = n.fy = undefined); 
    }},
    dagLevelDistance: {},
    dagNodeFilter: { default: node => true },
    onDagError: { triggerUpdate: false },
    nodeRelSize: { default: 4, triggerUpdate: false, onChange: notifyRedraw },
    nodeId: { default: 'id' },
    ...
    linkDirectionalArrowRelPos: { default: 0.5, triggerUpdate: false, onChange: notifyRedraw }, 
    linkDirectionalParticles: { default: 0 },
    linkDirectionalParticleSpeed: { default: 0.01, triggerUpdate: false }, 
    ...
    warmupTicks: { default: 0, triggerUpdate: false },
    ...
    tickFrame: function(state) {
      ...
      function paintLinks() {
        ...
        function calcLinkControlPoints(link) {
          ...
          const l = Math.sqrt(Math.pow(end.x - start.x, 2) + Math.pow(end.y - start.y, 2)); 
